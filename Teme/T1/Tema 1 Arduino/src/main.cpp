...
void loop() {
  ...
  else if (state == CHARGING)
  {
    ...
    if(current_charging_led == 4 || (stop_button_action == true && charging_time_elapsed > DEBOUNCE_STOP_DELAY))
    {
      state = ENDING;
  ...
  else if(state == ENDING)
  {
    ...
    if(ending_time_elapsed > 500 * ending_blink_helper)
    {
